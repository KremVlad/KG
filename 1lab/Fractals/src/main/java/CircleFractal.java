...
public class CircleFractal extends JPanel {

    public void paintComponent(Graphics g) {
        Graphics2D g2d = (Graphics2D) g;
        g.setColor(Color.RED);
        g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);   //сглаживание
        DrCircle(300, 300, 200, g);           //центр и размер фрактала
    }

    public void DrCircle(double x, double y, int size, Graphics g) {
        int N = 4, c = 3, rad = 0, den = 0;
        ...
        if (size > 0) {
            ...
            DrCircle(x, y, rad, g);           //центральная окружность
            for (int i = 0; i < N; i++) {     //все остальные окружности
                DrCircle(x - Math.round(den * Math.sin(2 * Math.PI / N * i)), y + Math.round(den * Math.cos(2 * Math.PI / N * i)), rad, g);
            }
